...
```cron
5 * * * * cd /opt/auto-social-infotainment && /usr/bin/flock -n state/extract.lock .venv/bin/workflow-automation extract-stories --source-id "$SOURCE_ID" >> logs/extract.out 2>> logs/extract.err
