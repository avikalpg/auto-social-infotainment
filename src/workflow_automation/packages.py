...
def create_content_package(
    ...
    media = ffprobe_validate(final_video, ffprobe_bin)
    ...
    atomic_json(
        package_dir / "manifest.json",
        {
            ...
            "files": {
                "caption": "caption.md",
                "publication_status": "publication-status.json",
                "final_video": "final-video.mp4",
            },
            "sha256": {"final_video": sha256_file(video_dest)},
            "media": {"final_video": media},
        },
    )
    validate_content_package(package_dir, ffprobe_bin)
    return package_dir


def validate_content_package(package_dir: Path, ffprobe_bin: str = "ffprobe") -> dict[str, Any]:
    ...
    video = package_dir / "final-video.mp4"
    if manifest.get("sha256", {}).get("final_video") != sha256_file(video):
        raise ValueError("final video sha256 mismatch")
    media = ffprobe_validate(video, ffprobe_bin)
    manifest_media = manifest.get("media", {}).get("final_video")
    if not manifest_media or manifest_media.get("sha256") != media["sha256"]:
        raise ValueError("final video ffprobe validation missing or stale")
